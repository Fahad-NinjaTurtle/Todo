import {
  ...
  startOfMonth, endOfMonth, isWithinInterval, addDays, addWeeks, addMonths, isWeekend,
} from 'date-fns';
import {
...
/** Local-timezone day key (yyyy-MM-dd) for an ISO timestamp or date string. */
...
function advance(date: Date, rule: string): Date {
  switch (rule) {
...
/** Next due date (yyyy-MM-dd) strictly after today, keeping the rule's cadence. */
export function nextDueDate(rule: string, fromDate: string | null): string {
  const today = todayISO();
  let base = parseISO(fromDate || today);
  if (isNaN(base.getTime())) base = parseISO(today);
  let next = base;
  for (let i = 0; i < 400; i++) {
    next = advance(next, rule);
    if (format(next, 'yyyy-MM-dd') > today) break;
  }
  return format(next, 'yyyy-MM-dd');
}

/** Next reminder timestamp (ISO) strictly in the future, keeping the rule's cadence. */
export function nextRemindAt(rule: string, fromISO: string): string {
  const nowMs = Date.now();
  let next = parseISO(fromISO);
...
export function computeDayQueue(state: AppState): DayQueueItem[] {
  ...
  state.reminders
    .filter(r => r.status === 'pending' && localDayKey(r.remindAt) <= now)
  ...
  const todaysHabits = state.habits
    ...
    .map(habit => ({
      ...
      completed: state.habitCompletions.some(
        c => c.habitId === habit.id && localDayKey(c.completedAt) === now
      ),
    }));
...
export function computeHabitStreak(habitId: string, completions: AppState['habitCompletions']): number {
  const dates = new Set(
    completions.filter(c => c.habitId === habitId).map(c => localDayKey(c.completedAt))
  );
  let streak = 0;
...
export function computeHabitCompletionRate(
  ...
  const dates = completions
    ...
    .map(c => localDayKey(c.completedAt));
