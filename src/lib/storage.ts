...
import { generateId, localDayKey } from './utils';
...
type MergeableEntity = { id: string; updatedAt?: string; createdAt?: string };

function mergeCollection<T extends MergeableEntity>(server: T[], client: T[]): T[] {
  ...
  for (const item of client) {
    ...
    const serverStamp = existing.updatedAt ?? existing.createdAt ?? '';
    const clientStamp = item.updatedAt ?? item.createdAt ?? '';
    if (clientStamp >= serverStamp) map.set(item.id, item);
  }
  return [...map.values()];
}

/**
 * Merges a newer server state with the local state after a sync conflict
 * (e.g. another device saved in between). Entities are unioned by id, with
 * the newer updatedAt winning. Local settings win.
 */
export function mergeStates(server: AppState, client: AppState): AppState {
  const activityMap = new Map(server.activity.map(a => [a.id, a]));
  for (const entry of client.activity) activityMap.set(entry.id, entry);
  const activity = [...activityMap.values()]
    .sort((a, b) => b.timestamp.localeCompare(a.timestamp))
    .slice(0, 2000);

  return {
    areas: mergeCollection(server.areas, client.areas),
    projects: mergeCollection(server.projects, client.projects),
    tasks: mergeCollection(server.tasks, client.tasks),
    inboxItems: mergeCollection(server.inboxItems, client.inboxItems),
    goals: mergeCollection(server.goals, client.goals),
    habits: mergeCollection(server.habits, client.habits),
    habitCompletions: mergeCollection(server.habitCompletions, client.habitCompletions),
    notes: mergeCollection(server.notes, client.notes),
    reminders: mergeCollection(server.reminders, client.reminders),
    waitingFor: mergeCollection(server.waitingFor, client.waitingFor),
    receivables: mergeCollection(server.receivables, client.receivables),
    payables: mergeCollection(server.payables, client.payables),
    expenses: mergeCollection(server.expenses, client.expenses),
    incomes: mergeCollection(server.incomes, client.incomes),
    visionItems: mergeCollection(server.visionItems, client.visionItems),
    weeklyReviews: mergeCollection(server.weeklyReviews, client.weeklyReviews),
    focusSessions: mergeCollection(server.focusSessions, client.focusSessions),
    trades: mergeCollection(server.trades, client.trades),
    activity,
...
export function computePomodoroStats(sessions: FocusSession[]): PomodoroStats {
  ...
  const sessionDays = new Set(workSessions.map(s => localDayKey(s.completedAt)));
  let streak = 0;
  ...
  for (const day of [...sessionDays].sort()) {
    run = prev !== null && format(addDays(parseISO(prev), 1), 'yyyy-MM-dd') === day ? run + 1 : 1;
