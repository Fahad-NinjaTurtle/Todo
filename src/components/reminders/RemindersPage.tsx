...
import { formatDateTime, PRIORITY_LABELS, RECURRENCE_OPTIONS } from '@/lib/utils';
                  ...
                  <div key={r.id} className="bg-surface border border-base rounded-2xl p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-indigo-400 mt-1">
                        ...
                        {r.recurrenceRule && <span className="text-muted"> · repeats {r.recurrenceRule}</span>}
                      </p>
                    </div>
...
function ReminderForm({ reminder, onSave, onClose }: { reminder: Reminder | null; onSave: (d: Omit<Reminder, 'id' | 'createdAt' | 'status'>) => void; onClose: () => void }) {
  ...
  const [recurrenceRule, setRecurrenceRule] = useState(reminder?.recurrenceRule ?? '');
  ...
  return (
    <Modal title={reminder ? 'Edit Reminder' : 'New Reminder'} onClose={onClose}>
      <form onSubmit={e => { e.preventDefault(); if (!title.trim() || !remindAt) return;
        onSave({ title: title.trim(), description, remindAt: new Date(remindAt).toISOString(), priority, recurrenceRule: recurrenceRule || null, linkedType: null, linkedId: null });
        ...
        <ModalBody>
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-medium text-secondary mb-1.5">Repeats</label>
              <select value={recurrenceRule} onChange={e => setRecurrenceRule(e.target.value)} className={FORM_SELECT}>
