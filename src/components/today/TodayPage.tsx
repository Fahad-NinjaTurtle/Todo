...
export default function TodayPage() {
  ...
  // Feedback after returning from the Google OAuth flow.
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const result = params.get('calendar');
    if (!result) return;
    if (result === 'connected') toast('Google Calendar connected — tasks will sync');
    else toast(`Calendar connection failed (${params.get('reason') ?? 'unknown'})`, 'error');
