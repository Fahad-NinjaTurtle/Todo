...
export default function Sidebar() {
  ...
  const inboxCount = state.inboxItems.filter(i => !i.processed).length;
        ...
        <div className="flex-1 min-h-0 overflow-y-auto os-scroll-sidebar py-2 px-2">
          ...
          {MAIN_NAV.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} title={collapsed ? label : undefined}
              ...
              {!collapsed && href === '/inbox' && inboxCount > 0 && (
                <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-accent-subtle text-accent">{inboxCount}</span>
              )}
            </Link>
          ))}
          ...
          {!collapsed && gcal?.configured && (
            gcal.connected ? (
              <div className="flex items-center gap-2 px-2.5 py-2 text-xs text-secondary">
                ...
                <span className="flex-1 truncate" title={gcal.email ?? undefined}>Calendar synced</span>
                <button onClick={disconnectGcal} className="text-[10px] text-muted hover:text-red-400">Disconnect</button>
              </div>
            ) : (
              <a href="/api/google/connect" className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-sm text-secondary hover:bg-raised hover:text-primary">
                <Calendar size={15} />
