...
import { PRIORITY_LABELS, TASK_STATUS_LABELS, FOCUS_QUEUE_LABELS, RECURRENCE_OPTIONS, RecurrenceRule } from '@/lib/utils';
...
export default function TaskForm({ task, defaultProjectId, defaultAreaId, defaultDueDate, defaultFocusQueue, defaultPriority, defaultTopPriority, onSave, onClose }: Props) {
  ...
  const [recurrenceRule, setRecurrenceRule] = useState<RecurrenceRule>(
    (task?.recurrenceRule as RecurrenceRule) ?? 'daily'
  );
          ...
          <div className="space-y-4">
            ...
            {isRecurring && (
              <div>
                ...
                <select value={recurrenceRule} onChange={e => setRecurrenceRule(e.target.value as RecurrenceRule)} className={FORM_SELECT}>
                  {RECURRENCE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                <p className="text-[11px] text-muted mt-1">Completing this task reschedules it to the next due date.</p>
              </div>
            )}
            <label className="flex items-center gap-2 text-sm text-secondary">
